...
public class AngleTracker implements Runnable {
	...
	int repet=0;
	boolean block=true; // interrupteur utile pour compter le nombre de répétition des ciseaux.
	...
	boolean isAlive = true; // Permettra d'arrêter la boucle infini.
	...
	public void printPosition() throws InterruptedException {
		...
		while (isAlive) { // permettra de sortir de la boucle en mettant isAlive a false.
			...
			if ( (Adj != Adjbis ) && (Ychip != 0) ) { // on evite de répéter les mêmes angles 
				...
				if(block==true && Adb >= 300){  // il faut débasser 300° pour avoir une répétition de plus
					repet=repet+1;
					block=false;				// on ouvre l'interrupteur pour éviter de compter plusieurs fois la répétition
				}

				if (block==false && Adb <= 50){  // il faut passer en dessous de 50° pour réouvrir l'interrupteur.
					block=true;
				}
				...
				if(i==29){
					try {
						...
						for ( int j=0;j<=119;j++){			// On remplit le vecteur avec 120 cofficients ce qui prend environ 1,5 secondes soit le temps d'unr répétition.
				...
				if(repet>=5){			// On ne gagne des sports coins qu'à partir de 5 répétitions.
